...
import { actionTypes } from "./../../../../redux/posts/posts_select_sidebar"
import { LinearProgress } from '@material-ui/core';
import { dateConvertMiladiToShamsi } from '../../../common/method/date';
import KeyboardArrowLeftIcon from '@material-ui/icons/KeyboardArrowLeft';

export function PostSidebarList() {
    const dataReducer = useSelector(state => state.reducerPostSidebar)
    ...
    useEffect(() => {
        dispatch({ type: actionTypes.postSidebarAsync, subgroup_name: location.state?.subgroup_name })
    }, [location]) //eslint-disable-line react-hooks/exhaustive-deps

    if (dataReducer.loading) {
        return (
            <div className="bg-white rounded-lg p-5 shadow m-5">
                <h4 className="border-bottom py-2 text-center">پست ها مرتبط </h4>
                <LinearProgress />
            </div>
        )
    }
